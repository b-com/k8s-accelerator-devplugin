...
func NewAcceleratorDevicePlugin(acceldevs intelPorts) *AcceleratorDevicePlugin {
	var devs []*pluginapi.Device
	for _, port := range acceldevs.portList {
		// AFUs BDF (bus,dev,fn) are used as IDs rather than instance IDs so the container can get BDF from env var ACCELERATOR_DEVICES
		// when FPGA will manage several SRIOV virtual functions, the container appli will have to filter fpgaEnumerate() with this BDF
